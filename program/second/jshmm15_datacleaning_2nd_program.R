...
m_data$初回サルベージ奏効_初回サルベージ生存 <-
  ifelse(
    as.Date(m_data$初回サルベージ奏効) - as.Date(m_data$初回サルベージ生存) <=0, "", as.Date(m_data$初回サルベージ奏効) - as.Date(m_data$初回サルベージ増悪)
  )
...
m_data$初期治療最終投与_初回PD判定1年確認 <-　ifelse(
  as.Date(m_data$初期治療最終投与) - as.Date(m_data$初回PD判定) >= 365, "←", ""
)
m_data$初回サルベージ奏効_初回サルベージ増悪_SD以上 <- ifelse(
                                          m_data$初回サルベージ中の奏効.best.response. %in% c("CR", "PR", "SD", "sCR", "VGPR") & m_data$初回サルベージ治療後の初回再発または増悪の有無 == "あり" & m_data$初回サルベージ奏効_初回サルベージ増悪 >=0, "←", ""
)
